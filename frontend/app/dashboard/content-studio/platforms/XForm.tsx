...
import { XPost, MediaType } from '../types';
...
export const XForm: React.FC<XFormProps> = ({
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                ...
                >
                    <input
                        ...
                        accept={post.mediaType === 'video' ? 'video/*' : post.mediaType === 'gif' ? 'image/gif' : 'image/*'}
                        multiple={post.mediaType === 'image'}
                    />
                    {renderUploadPreview()}
                </div>
                <p className="mt-1 text-xs text-gray-500">
                    {post.mediaType === 'image' ? 'Up to 4 images allowed' : 
                     post.mediaType === 'gif' ? 'Single GIF allowed' : 
            ...
            {/* Quote Tweet */}
            <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Quote Tweet ID</label>
                <input
                    type="text"
                    ...
                    <div className="space-y-3">
                        <div className="flex gap-2">
                            ...
                            <button
                                ...
                                disabled={!pollOption.trim() || (post.poll?.options.length || 0) >= 4}
                        ...
                        {post.poll && (
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Poll Duration</label>
                                <select
                                    value={post.poll.durationMinutes}
                                    onChange={(e) => onInputChange('poll', { ...post.poll, durationMinutes: Number(e.target.value) })}
                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                                >
