...
export const XPreview: React.FC<XPreviewProps> = ({ post }) => {
  ...
  const brandUsername = brand?.brand_name?.toLowerCase().replace(/\s+/g, '') || "yourbusiness";
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl max-w-xl mx-auto w-full">
      <div className="flex items-start p-3 sm:p-4">
        <div className="h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-gray-200 flex-shrink-0 overflow-hidden">
          {brandLogo ? (
            <img
              src={brandLogo}
              alt={brandName}
        ...
        <div className="ml-2 sm:ml-3 flex-grow min-w-0">
          <div className="flex items-center flex-wrap gap-1">
            <p className="font-bold text-sm sm:text-base">{brandName}</p>
            <svg
